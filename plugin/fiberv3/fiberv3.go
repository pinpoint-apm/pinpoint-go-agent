...
func wrap(f func(c fiber.Ctx) error, handlerName string) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		tracer := pphttp.NewHttpServerTracerWithReader(
			...
			serverName,
			fiberRequestHeader{&c.RequestCtx().Request.Header},
		)
		...
		if tracer.IsSampled() {
			pphttp.RecordHttpServerRequestWithReader(tracer,
				...
				fiberRequestHeader{&c.RequestCtx().Request.Header}, fiberCookie{&c.RequestCtx().Request.Header})
		}
...
func recordResponse(tracer pinpoint.Tracer, c fiber.Ctx, status int) {
	pphttp.RecordHttpServerResponseWithReader(tracer, status, fiberResponseHeader{&c.RequestCtx().Response.Header})
}
