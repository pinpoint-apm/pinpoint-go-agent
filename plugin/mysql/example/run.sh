...
echo -e "\n${YELLOW}[2/6] Building MySQL Docker image...${NC}"
docker build -t mysql-pinpoint-test .
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if docker exec $CONTAINER_NAME mysqladmin ping -h localhost -uroot -pp123 --silent 2>/dev/null; then
...
if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    echo -e "\n${RED}Error: MySQL failed to start within expected time${NC}"
...
echo "Building mysql_example..."
go build -o mysql_example mysql_example.go

# Run the application in background
echo "Starting application on http://localhost:9001"
./mysql_example &
APP_PID=$!
...
echo ""
echo "Test endpoints:"
echo "  curl http://localhost:9001/tableCount"
echo "  curl http://localhost:9001/query"
echo ""
