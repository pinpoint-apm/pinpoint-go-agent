...
func WrapPartitionConsumer(pc sarama.PartitionConsumer, agent *pinpoint.Agent) *PartitionConsumer {
	...
	apiId := agent.RegisterSpanApiId("Kafka Consumer Invocation", pinpoint.ApiTypeWebRequest)
	...
	go func() {
		...
		for msg := range msgs {
			...
			tracer := agent.NewSpanTracerWithReader("kafka.consume", reader)

			tracer.Span().SetServiceType(serviceTypeKafkaClient)
			tracer.Span().SetApiId(apiId)
			tracer.Span().SetRpcName(makeRpcName(msg))
