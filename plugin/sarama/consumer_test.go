...
// The key used to be the untyped string constant "ppsarama.broker.address", so any package
...
func TestNewContext_ForeignStringKeyDoesNotShadowTheAddresses(t *testing.T) {
	...
	ctx = context.WithValue(ctx, "ppsarama.broker.address", "not a slice") //nolint:staticcheck // the point of the test
