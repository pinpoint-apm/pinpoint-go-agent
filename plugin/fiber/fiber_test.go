...
func TestMiddleware_PutsSampledTracerInUserContext(t *testing.T) {
	startAgent(t)
...
func TestMiddleware_KeepsExistingUserContextValues(t *testing.T) {
	...
	app.Use(func(c *fiber.Ctx) error {
		c.SetUserContext(context.WithValue(c.UserContext(), ctxKey{}, "from-auth-middleware"))
		return c.Next()
	})
	app.Use(Middleware())
	app.Get("/", func(c *fiber.Ctx) error {
		gotUser = c.UserContext().Value(ctxKey{})
		gotTracer = pinpoint.FromContext(c.UserContext())
		return nil
	})
