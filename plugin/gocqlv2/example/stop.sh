...
echo -e "${YELLOW}Stopping Cassandra (gocql/v2) Pinpoint Test Environment...${NC}"

# Stop Go application if running
if [ -f "gocqlv2_example" ]; then
    echo "Stopping Go application..."
    pkill -f "gocqlv2_example" 2>/dev/null || true
    rm -f gocqlv2_example
