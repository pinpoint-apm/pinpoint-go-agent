...
| Absent or refused optional proxy fields (`D=`, `i=`, `b=`) | `ProxyRequestHeaderBuilder` defaults (-1) | **Same as Java** — recorded as -1, which the web UI reads as "not reported", rather than as a 0 it cannot tell from a measured zero. The C++ agent records the same -1 (`kProxyUnset` in `src/http.cpp`); locked in group 14 |
| Proxy header recording switch | `profiler.proxy.http.header.enable` (`DefaultRequestRecorderFactory`) | **Adopted** — `Http.Server.ProxyHeaderEnable` (default true, dynamic) gates all four proxy header kinds. The C++ agent has the same key with the same default (`http.server.proxy_header_enable`, checked in `HttpTracerUtil::setProxyHeader`) |
| Real-IP header resolution | `RealIpHeaderResolver`, `profiler.server.realipheader`, `profiler.server.realipemptyvalue` | **Same algorithm as Java, different default** — `Http.Server.RealIpHeader` defaults to `["X-Forwarded-For", "X-Real-Ip"]` for compatibility where Java trusts no header; see [config.md](config.md#httpserverrealipheader). The C++ agent keeps the same default |
| Request parameter recording | `HttpServletParameterExtractor`, `profiler.server.tracerequestparam`, `ClientRequestRecorder` / `InterceptorUtils.getHttpUrl`, `profiler.<plugin>.param` | **Same format as Java, opt-in instead of default on** — `Http.Server.RecordRequestParam` and `Http.Client.RecordUrlQuery` default to `false` because query strings carry tokens and ids; see [config.md](config.md#httpserverrecordrequestparam) and [config.md](config.md#httpclientrecordurlquery). The C++ agent made the same choice |
...
warning when the stat stream is not draining. Distinct URIs per tick default
to Java's 1000 (`Http.UrlStat.LimitSize`; Java
`profiler.uri.stat.completed.data.limit.size`) in both ports. The input
queue stays at 1024 (`Http.UrlStat.QueueSize`) where Java's
`UriStatStorageProvider` hard-codes 5192: here it is one channel, in C++ 16
shards of 1024, and neither port's shape reproduces a single 5192-entry queue
faithfully (the C++ agent's file records the same decision). URL statistics
stay off by default in
...
**Fixed as a port consensus.** Because the policy is a three-way split (Java
retries, both ports drop), it is locked as group 17 of the parity invariants in
both suites (`Test_javaParityLock_MetadataRetryBudget` here,
`MetadataRetryBudget` in the C++ suite), so a change in either port cannot
silently leave the two disagreeing; a change of policy is a change in both
ports, made together.
